...
param2_callName:(param2_type)param2_varName;
  @MiPhoneOS1_ObjectiveC classname : superclassname {
    // Start of script
    // Warning: MacOS and iOS programming differs. This script has a higher chance of not working due to non-existant mobile libraries
    // Connecting iPhone OS 1 applications together (may not be possible, due to lack of app support on iPhone OS 1) Note that I have very little experience with Objective-C, so this script likely doesn't work correctly, and is currently just a demo script
    //Definitions
    ...
    app2URL = string("/I don't know the iPhoneOS File path/"); // Incomplete. This method is for linking the 2 apps together, and jumping to the other program. Will prompt an install notice if it fails to reach. If the program is installed but reaching it results in an error, that means that ProTraApp was configured incorrectly
    // Main
    @switchPrograms
...
@end
/* File info
* File version: `1 (Thursday, March 11th 2021 at 8:21 pm)
