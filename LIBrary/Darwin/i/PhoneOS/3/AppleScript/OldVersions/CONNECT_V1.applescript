...
-- Connecting 2 iPhone OS 3 applications together. Note that I have very little experience with AppleScript, so this script likely doesn't work correctly, and is currently just a demo script
-- Definitions
class iPhoneOS_3 -- Trying to form a class here, so it can detect from different iPhone OS 1 App versions.
...
app2URL = string("/I don't know the iPhoneOS 3 File path/") -- Incomplete. This method is for linking the 2 apps together, and jumping to the other program. Will prompt an install notice if it fails to reach. If the program is installed but reaching it results in an error, that means that ProTraApp was configured incorrectly
-- Main
...
-- End of main method
-- File info
-- File version: `1 (Thursday, March 11th 2021 at 8:56 pm)
